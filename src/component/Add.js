...
//import AddReducer from "../store/Adddetail"
...
const Add = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const checkEmail = Adddetals.find((Adds) => Adds.email === email && email );
    const checkUser = Adddetals.find((Adds) => Adds.user_name === user_name && user_name );
    ...
    const data = {
        id: Adddetals[Adddetals.length -1].id +1,
        user_name,
        name,
        email,
        xenderOf,
    ...
    dispatch(addActions.add_details(data));
